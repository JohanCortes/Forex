...
if "__main__"==__name__:
    xls = pd.ExcelFile('assets/ForexMarket.xlsx') # leer archivo excel
    ...
    f,c = xi.shape
    fac_ap = 0.25 # Factor de aprendizaje
    ...
    n_ocultas = 3 # Neuronas en la capa oculta
    ...
    def err():
        ...
        x = np.arange(epochs)
        plt.plot(x,E,'g',label="Error global")
        plt.legend(loc='upper right')
        plt.show()
    
    # validacion
    col=0.00674101593364469
    ...
    # divisas utilizadas
    forex=['EURUSD','GBPUSD','USDBRL','USDCAD','USDCHF','USDCZK','USDJPY','USDNOK','USDPLN','USDRUB','USDSEK','USDTRY','USDZAR']   
    # graficar las salidas
    def sal():
        ax = plt.subplot(2,1,1)
        plt.grid()   
        ax.set_ylabel("Rendimiento",fontsize=12)
        ax.set_title("Rendimiento mensual",fontsize=14)
        ax.plot(forex,salidas,'r',label="R. Divisas")
        ax.plot([forex[0],forex[-1]],[col,col],'#CC0',label='R. USDCOP')
        ax.legend(loc='upper right')
        #plt.show()
    
    maximo=max(col,max(salidas)[0]) 
    minimo=min(col,min(salidas)[0])
    dis=max((col-minimo),(maximo-col)) # distancia maxima entre rendimiento de las divisas y el rendimiento USDCOP
    similitud={}
    for e in salidas:
        dif=np.abs(col-e[0])
        similitud[forex[salidas.index(e)]]=(1-(dif/dis))*100 # porcentaje de similitud de cada divisa con USDCOP
    print("Pordentaje de similitud: ",similitud)
    # graficar la similitud
    def sim():
        ax = plt.subplot(2,1,2)
        ax.set_ylabel("P. Similitud",fontsize=12)
        ax.set_xlabel("Divisas",fontsize=12)
        ax.set_title("Similitud general",fontsize=14)
        ax.grid(axis="y")
        ax.bar(list(similitud),similitud.values())
        #plt.show()
    sal()
    sim()
    plt.show()
